...
{
public:
  unicode_string(size_t msize=256)
  :max_size(msize), current_size(0) {
    buffer = new u_char[msize];
  }
  virtual ~unicode_string() { delete[] buffer; }
  ...
  void push_back(unicode_variant variant, char ch) {
    if (current_size < max_size) {
      buffer[current_size].variant = variant;
    }
  }
  
  size_t size() { return current_size; }
  void clear() { current_size = 0; }
  friend ostream& operator<<(ostream& stream, const unicode_string& str) const;
...
ostream& operator<<(ostream& stream, const unicode_string& str) const {
  stream << str.buffer;
}
